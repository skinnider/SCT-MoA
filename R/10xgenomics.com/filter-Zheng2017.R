# Filter the ten human bead-enriched PBMC subpopulations described by
...
exprFiles = list.files("data/10xgenomics.com/processed",
                       pattern = "*.gz", full.names = T)
for (exprFile in exprFiles) {
  accession = gsub("\\..*$", "", basename(exprFile))
  output_file = paste0("data/10xgenomics.com/filtered/", accession, ".txt")
